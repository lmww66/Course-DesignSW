...
static void dso(unsigned char data,unsigned char delta)
{
	...
	showcnt=showindex;
	for(cnt=0;  cnt <=127/delta ;cnt++)//��Ļ����0-127
	{
		draw_point(127-(cnt*delta),buf[showcnt],1);
		showcnt=showcnt-point_delta;
		if(showcnt>=256)
		{
			showcnt=256-point_delta+showcnt;//�ص�����β��
		}
	}
	//�ѵ�������
	...
	for(cnt=0;  cnt <=127/delta ;cnt++)
	{
		...
		if(next_line_showindex>=256)
		{
			next_line_showindex=256-point_delta+next_line_showindex;
		}
		...
		if(line_showindex>=256)
		{
			line_showindex=256-point_delta+line_showindex;
		}
	}
