...
void time_init()
{
	...
	timeDate.time.hour = 12;
	timeDate.time.ampm = CHAR_24;
	timeDate.date.day = DAY_SAT;
	timeDate.date.date = 4;
	timeDate.date.month = MONTH_JUL;
	timeDate.date.year = 26;
	time_wake();
}
	...
	if(++timeDate.time.secs >= 60)
	{
		...
		if(++timeDate.time.mins >= 60)
		{
			...
			if(++timeDate.time.hour >= 24)
			{
				...
				if(++timeDate.date.date > time_monthDayCount(timeDate.date.month, timeDate.date.year))
				{
					timeDate.date.date = 1;
					if(++timeDate.date.month > 11)
					{
						timeDate.date.month = 0;
						timeDate.date.year++;
					}
					timeDate.date.day = time_dow(timeDate.date.year, timeDate.date.month, timeDate.date.date);
				}
				else
				{
					timeDate.date.day = (timeDate.date.day + 1) % 7;
				}
			}
		}
	}
