...
void watchface_normal()
{
	...
	buttons_setFuncs(menu_select, menu_select, my_menu_open);
	animation_start(NULL, ANIM_MOVE_ON);  //���ô򿪹��ȶ���������ִ�к�����
}
...
//	if(CHARGING())
	...
	// Draw step count
	{
		char stepBuff[16];
		if (dmp_ready)
			sprintf(stepBuff, "Step:%lu", step_count);
		else
			sprintf(stepBuff, "Step:---");
		draw_string(stepBuff, false, 0, 48);
	}
...
static display_t ticker()
{
	...
	// Draw colon for half a second   �������ð��
	 if(millis()%1000>500)
